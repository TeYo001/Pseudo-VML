...
void section_push_back(ExeInfo* exe_info, ModTable* mod_table, SectionBuildInfo* new_section, IMAGE_SECTION_HEADER* new_header) {   
    ...
    add_mod_entry_replace(mod_table, 
            exe_info->end_of_header_offset + (exe_info->nt_header->FileHeader.NumberOfSections) * SECTION_TABLE_ENTRY_SIZE,
...
// msfvenom -c messageBox -a x86_64 --platform windows -p windows/x64/messagebox TEXT="We are evil" -f raw > ./test/coolbeans.txt
